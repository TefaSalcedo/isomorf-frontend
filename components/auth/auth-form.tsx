...
function Brand() {
  ...
  return (
    <Link href="/" className="flex items-center gap-3">
      ...
      <span>
        <span className="block text-lg font-bold tracking-tight text-slate-950">{t('appName')}</span>
        <span className="block text-[10px] font-bold uppercase tracking-[.16em] text-violet-600">{t('tagline')}</span>
      </span>
    </Link>
  );
}
      ...
      <div className="relative my-8 grid min-h-64 place-items-center overflow-hidden rounded-2xl border border-white/15 bg-[#17142b] shadow-2xl">
        ...
        <div className="relative h-48 w-40 rotate-[-8deg] rounded-t-[40%] border-4 border-violet-300/70 bg-gradient-to-br from-slate-100/70 to-violet-300/20 shadow-[0_0_50px_rgba(139,92,246,.3)]">
          ...
          <div className="absolute -left-9 top-10 rounded-xl border border-violet-400/40 bg-slate-950/80 px-3 py-2 text-[10px] shadow-xl">
            <strong className="block text-violet-200">{t('beamLabel')}</strong><span className="text-slate-400">D/C 0.74</span>
          </div>
          <div className="absolute -right-14 bottom-8 rounded-xl border border-emerald-400/30 bg-slate-950/85 px-3 py-2 text-[10px] shadow-xl">
            <strong className="block text-emerald-300">{t('femLabel')}</strong><span className="text-slate-400">{t('withinTolerance')}</span>
          </div>
        </div>
      </div>
      <div className="relative flex items-center justify-between border-t border-white/10 pt-5 text-xs text-violet-100/70">
        <span>{t('footer')}</span><span className="font-semibold text-amber-300">★★★★★</span>
      </div>
    </div>
  );
}
    ...
    <main className="min-h-screen bg-[#faf8fd] px-4 py-5 text-slate-900 sm:px-8 lg:px-10">
      ...
      <section className="mx-auto my-10 grid max-w-[1360px] overflow-hidden rounded-3xl border border-violet-100 bg-white shadow-[0_18px_60px_rgba(76,29,149,.10)] lg:grid-cols-12">
        <div className="px-7 py-10 sm:px-12 lg:col-span-7 lg:px-14 lg:py-14">
          <span className="inline-flex items-center gap-2 rounded-full bg-violet-50 px-3 py-1.5 text-xs font-semibold text-violet-700"><span className="h-2 w-2 rounded-full bg-emerald-400" />{mode === 'login' ? t('badgeLogin') : t('badgeRegister')}</span>
          <h1 className="mt-6 text-3xl font-bold tracking-tight text-slate-950 sm:text-4xl">{mode === 'login' ? t('titleLogin') : t('titleRegister')}</h1>
          <p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">{mode === 'login' ? t('subtitleLogin') : t('subtitleRegister')}</p>
          <div className="mt-8 flex gap-3"><SsoButton>{t('continueGoogle')}</SsoButton><SsoButton>{t('continueMicrosoft')}</SsoButton></div>
          <div className="my-7 flex items-center gap-3 text-[11px] font-bold uppercase tracking-wider text-slate-400"><span className="h-px flex-1 bg-slate-200" />{t('orUseEmail')}<span className="h-px flex-1 bg-slate-200" /></div>
          <form onSubmit={submit} className="space-y-4">
            {mode === 'register' && <div className="grid gap-4 sm:grid-cols-2"><Field name="first_name" label={t('firstName')} placeholder="Thomas" /><Field name="last_name" label={t('lastName')} placeholder="Sanchez" /></div>}
            <Field name="email" label={mode === 'login' ? t('emailLogin') : t('emailRegister')} placeholder={t('emailPlaceholder')} type="email" icon={Mail} />
            <div><div className="mb-2 flex items-center justify-between"><label className="text-xs font-bold uppercase tracking-wide text-slate-600" htmlFor="password">{t('password')}</label>{mode === 'login' && <Link className="text-xs font-semibold text-violet-600" href="/forgot-password">{t('forgotPassword')}</Link>}</div><div className="relative"><LockKeyhole className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input id="password" required minLength={8} type="password" name="password" placeholder={t('passwordPlaceholder')} className="h-12 w-full rounded-xl border border-slate-200 bg-slate-50 pl-11 pr-4 text-sm outline-none transition placeholder:text-slate-400 focus:border-violet-500 focus:bg-white focus:ring-4 focus:ring-violet-500/10" /><Eye className="absolute right-4 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" aria-hidden="true" /></div></div>
            {mode === 'register' && <label className="flex items-start gap-2 text-xs leading-5 text-slate-500"><input type="checkbox" required className="mt-1 accent-violet-600" />{t.rich('acceptTerms', { link: (chunks) => <a className="font-semibold text-violet-600" href="#terms">{chunks}</a> })}</label>}
            {mode === 'login' && <div className="flex items-center justify-between text-xs text-slate-500"><label className="flex items-center gap-2"><input type="checkbox" className="accent-violet-600" />{t('keepSignedIn')}</label><span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-1 font-semibold text-emerald-700"><ShieldCheck className="h-3 w-3" /> {t('secureSession')}</span></div>}
            {error && <p className="rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-600">{error}</p>}
            <button disabled={pending} className="flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-violet-600 to-purple-600 text-sm font-bold text-white shadow-lg shadow-violet-500/25 transition hover:from-violet-700 hover:to-purple-700 disabled:opacity-50">{pending ? t('processing') : mode === 'login' ? t('submitLogin') : t('submitRegister')}<ArrowRight className="h-4 w-4" /></button>
          </form>
          <div className="mt-8 flex items-center justify-between border-t border-slate-100 pt-5 text-xs text-slate-400"><span className="inline-flex items-center gap-1"><Check className="h-3 w-3 text-emerald-500" />{t('tlsEncrypted')}</span><span>{t('supportCenter')}</span></div>
        </div>
        <Showcase />
      </section>
      <footer className="mx-auto flex max-w-[1360px] justify-between text-xs text-slate-400"><span>{t('footerCopyright')}</span><span>{t('footerLinks')}</span></footer>
    </main>
  );
}
