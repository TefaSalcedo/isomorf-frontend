...
import { localeTags } from '@/lib/i18n/messages';
import type {
  BeamElement,
  BeamProperties,
  ColumnElement,
...
export function MemberDesignPanel({
  ...
  function elementLabel(target: DesignableElement): string {
    const kind = target.element_type === 'column' ? t('column') : t('beam');
    return `${kind} ${target.id.slice(0, 8)}`;
  }
  ...
  function calculate() {
    const result: DesignMemory =
      element.element_type === 'column'
        ? columnDesign(element, concrete, reinforcement, loads, locale)
        : beamDesign(element, concrete, reinforcement, loads, locale);
    patch({ concrete, reinforcement, design_loads: loads, design_memory: result });
  }

  function download() {
    if (!memory) return;
    const markdown = memoryToMarkdown(memory, { projectName, elementLabel: elementLabel(element) }, locale);
  ...
  useEffect(() => {
    if (!showFullMemory) return undefined;
    closeButtonRef.current?.focus();
    function onKeyDown(event: KeyboardEvent) {
      if (event.key === 'Escape') setShowFullMemory(false);
    }
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto p-4">
      ...
      {memory && (
        ...
        >
          <div className="flex items-center gap-1.5 font-semibold">
            ...
            {stale ? t('stale') : memory.status === 'ok' ? t('statusOk') : t('statusReview')}
          </div>
          <p className="mt-1">
            {stale
              ? t('staleBody')
          ...
          <p className="mt-1 text-[10px] opacity-80">
            {new Date(memory.calculated_at).toLocaleString(localeTags[locale])} · D/C {memory.ratio}
          </p>
        </div>
      )}
      ...
      <Section title={t('geometry')}>
        {element.element_type === 'column' ? (
          <>
            <Field label={t('widthB')} step={0.05} value={element.properties.width} onChange={(width) => patch({ width })} />
            <Field label={t('depthH')} step={0.05} value={element.properties.depth} onChange={(depth) => patch({ depth })} />
            <Field label={t('height')} step={0.1} value={element.properties.height} onChange={(height) => patch({ height })} />
          </>
        ) : (
          <>
            <Field label={t('widthB')} step={0.05} value={element.properties.width} onChange={(width) => patch({ width })} />
            <Field label={t('depthH')} step={0.05} value={element.properties.height} onChange={(height) => patch({ height })} />
            <Field label={t('spanL')} step={0.1} value={element.properties.length} onChange={(length) => patch({ length })} />
          </>
        )}
      </Section>

      <Section title={t('concreteSteel')}>
        <Field label={t('fc')} value={concrete.fc} onChange={(fc) => patch({ concrete: { ...concrete, fc } })} />
        <Field label={t('fy')} value={concrete.fy} onChange={(fy) => patch({ concrete: { ...concrete, fy } })} />
        <Field label={t('cover')} step={0.005} value={concrete.cover} onChange={(cover) => patch({ concrete: { ...concrete, cover } })} />
      </Section>
      ...
      <Section title={t('reinforcement')}>
        ...
        <Field label={t('stirrupSpacing')} step={0.01} value={reinforcement.stirrup_spacing} onChange={(stirrup_spacing) => patch({ reinforcement: { ...reinforcement, stirrup_spacing } })} />
      </Section>

      <Section title={t('externalLoads')}>
        {element.element_type === 'column' ? (
          <Field label={t('axialPu')} value={loads.axial} onChange={(axial) => patch({ design_loads: { ...loads, axial } })} />
        ) : (
          <Field label={t('distributedWu')} value={loads.distributed} onChange={(distributed) => patch({ design_loads: { ...loads, distributed } })} />
        )}
      </Section>
      ...
      {memory && (
        <>
          <Section title={stale ? t('memoryStaleTitle') : t('memoryTitle')}>
